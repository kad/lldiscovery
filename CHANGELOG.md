...
### Fixed
- **nwdiag Export Spurious P2P Networks**: Fixed nwdiag export creating many bogus point-to-point networks for edges between nodes that are already in the same segment. Now correctly marks only edges that share the segment's network prefixes as processed, preventing them from being exported as separate p2p networks. Edges on different VLANs between segment members are correctly shown as P2P networks.
- **nwdiag P2P Network Prefixes**: P2P networks in nwdiag now show actual network prefixes in the address field (e.g., "10.0.3.0/24, fd66:1f7:10::/64 (1000 Mbps)") instead of just "P2P (1000 Mbps)". Provides complete network information for point-to-point links.
...
- **2-Node Remote Segment Detection**: Remote segments with only 2 nodes are now detected when they share a common network prefix. Previously required 3+ nodes for remote segments, but VLANs with only 2 participating hosts were being missed. Now checks for shared network prefixes (e.g., 10.0.3.0/24) to identify legitimate 2-node segments, distinguishing them from simple P2P links.

### Fixed
- **Complete Intra-Segment Edge Hiding**: Fixed edge hiding logic to properly detect ALL interfaces on segment nodes, not just the primary interface from EdgeInfo. Previously, nodes with EdgeInfo entries would only have one interface added to the hiding map, causing edges from their secondary interfaces (WiFi) to remain visible. Now checks all nodes for additional interfaces with matching prefixes, ensuring complete edge hiding for multi-homed nodes. Example: fork's WiFi (wlp0s20f3) edges to other segment_0 members are now properly hidden.
- **WiFi Segment Visualization**: Fixed segment-to-node connections in DOT export to show ALL interfaces on each node that are part of the segment, not just the primary interface. Previously, nodes with both wired and WiFi interfaces on the same segment only showed the wired connection. Now correctly identifies and visualizes all interfaces with matching network prefixes, showing complete multi-homing topology. For example, a node with both enp0s31f6 (wired, 1 Gbps) and wlp0s20f3 (WiFi, 100 Mbps) on segment_0 now shows both connections to the segment ellipse.
- **WiFi Intra-Segment Edge Hiding**: Fixed edge hiding logic to properly hide ALL edges between segment members, including WiFi-to-wired and WiFi-to-WiFi connections. Previously, edge marking was asymmetric (only marking one interface per direction), causing some WiFi edges within segments to remain visible. Now marks both interfaces bidirectionally in segmentEdgeMap, ensuring complete edge hiding regardless of source/destination order or interface type combination.
- **Secondary Segment Connections**: Fixed visualization of multi-homed nodes where secondary interfaces on the same segment were shown as individual P2P edges instead of being hidden as intra-segment edges. Nodes with multiple interfaces (e.g., wired + WiFi) on the same segment now have all their connections properly recognized. Changed DOT export edge hiding logic to track ALL interfaces per node (not just one), using prefix matching to identify which interfaces belong to each segment. Result: Clean topology with multi-interface segment membership correctly visualized.
